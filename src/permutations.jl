...
function Base.one(P::Perm{N})::Perm{N} where N
	error("identity not implemented for $(typeof(P))")
end
...
Base.one(P::Cyc{N}) where N = Cyc{N}(0, :id)
Base.one(T::Type{Cyc{N}}) where N = Cyc{N}(0, :id)
